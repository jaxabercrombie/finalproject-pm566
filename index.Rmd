...
2.  How do vaccination efforts vary by vaccine company (Pfizer, Moderna, Johnson & Johnson) and age groups?
...
demoAgeCA <- read.csv("CA Vaccine Data Demographic Age.csv")
...
# Make other dataset up until November 30 as well for consistency
demoAgeCA <- 
  demoAgeCA %>%
  mutate(date = as.Date(demoAgeCA$date, "%m/%d/%Y")) %>%
  filter(date <= as.Date("2021-11-03"))
...
fig4 <-
  ...
  ggplot() +
  geom_line(aes(x = date, y = ave_fully_vax_perc, color = demographic_value)) +
  scale_color_manual(values = c("#49A1EA", "#3D8ACA", "#3171A6", "#295E8A")) +
  labs(title = "Fully Vaccinated Individuals by Age Group", x = "Date", 
         y = "Percent Fully Vaccinated") +
  theme(legend.title = element_blank(), axis.title.y = element_text(hjust = -1.5)) +
  facet_wrap(~region)

ggplotly(fig4, tooltip = c("x", "y", "demographic_value"))
```
Especially since the health impacts of COVID-19 vary on the basis of age, with the elderly having more adverse effects than youth, seeing where gaps in vaccination efforts lie is crucial. These plots follow similar shapes, since eligibility began with older populations and eventually opened up to the youngest group over the year. Age groups after the 65+ age group appear to have steeper slopes, demonstrating a demand for vaccines and an available supply after majority of the elderly population received their doses. In almost all regions, the 18-49 age group, despite being the largest age range, has the highest percent of fully vaccinated individuals. It appears data is missing for the 12-17 group for Central California, but nevertheless it likely follows the trend of the other three regions. Again, the most vaccinated regions are the Bay Area and Southern California.
